...
namespace pcre2 {
  ...
  : pattern{pattern},
    ...
    std::cout << "MatchContext::MatchContext(const Pattern&, const std::string&, size_t, size_t, uint32_t) -> " << this << "<match_data=" << &match_data << ">" << std::endl;
  }

  MatchContext::~MatchContext() {
    std::cout << "MatchContext::~MatchContext() -> " << this << "<match_data=" << &match_data << ">" << std::endl;
  ...
  : pattern{std::move(other.pattern)},
    ...
    match_data = nullptr;
    ovector = nullptr;
    std::cout << "MatchContext::MatchContext(MatchContext&&) -> " << this << "<match_data=" << &match_data << ">" << std::endl;
  }

  Match MatchContext::to_match() {
    return Match(*this);
  }
